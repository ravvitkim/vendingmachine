...
public class InputValidation {
    ...
    public void cardNumberCheck(String cardNumber) throws MyException {
        if (!cardNumber.matches("^\\d{4}-\\d{4}-\\d{4}-\\d{4}$")) {
            throw new MyException("✔카드번호 입력방식은 xxxx-xxxx-xxxx-xxxx 형식입니다.");
        }
    }


    public void rechargeCheck(int recharge) throws MyException {
        if (recharge <= 0) {
            throw new MyException("충전 금액은 0보다 커야 합니다.");
        }
        if (recharge % 1000 != 0) {
